...
// 페이지네이션을 위한 변수
let limit = 10; // 화면에 보여질 리스트 개수를 10개로 할당
let page = 1; // 왼쪽/오른쪽의 버튼을 눌렀을시에 이동을위한 값?

// ul 요소에 data 배열의 모든 데이터를 화면에 렌더링합니다.
const ul = document.querySelector("ul.discussions__container");
render(ul, 0, limit);

const getPageStartEnd = (limit, page) => { // 화면에 렌더링할 값과
...
buttons.children[0].addEventListener("click", () => { // 부모요에 클릭이벤트가 발생시에!!
  ...
  if (page > 1) { // page의 값이 1보다작다면
    page = page - 1; // page의 값에서 -1을 해준다?
  }
  // 그게 아니라면
